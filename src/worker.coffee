...
fetch_url = (url, response, this_inst, parse_delay, request_headers, get_requests, get_cookies) ->
  ...
  # received this callback is triggered, we should only store if the URL is final.
  # However, there appear to be rare cases where onUrlChanged is not triggered until
  # *after* all resources have been received, resulting in no headers being returned
  # at all, so let's store everything until that case can be figured out.
  page_inst.onResourceReceived = (response) ->
    decoded_url = decodeURIComponent(response.url)
    headers[decoded_url] = response.headers if response.stage is "end"
